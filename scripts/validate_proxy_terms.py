...
Output:
    scripts/output/correlation_report.json
    scripts/output/overlay_trends_vs_prediction.png
    scripts/output/per_app_correlation.png
    scripts/output/residuals.png
    scripts/output/cross_correlation.png
    scripts/output/*.csv (raw data exports)
...
OUTPUT_DIR = Path(__file__).parent / "output_proxy"
...
# Primary search terms — in-app activity proxies (profile optimization, paid features)
PRIMARY_TERMS = ["tinder gold", "phrase d'accroche", "premier rendez-vous", "ghosting"]

# Variant search terms — downstream activity signals
VARIANT_TERMS = [
    "bio tinder",
    "super like",
    "tinder boost",
    "premier message tinder",
    "idee premier rendez-vous",
    "bar date",
]
...
def main():
    ...
    if not trends_data:
        ...
        try:
            ...
            print("[HINT] If you have cached CSVs, place them in scripts/output/gt_raw_*.csv")
            sys.exit(1)
    else:
        print(f"\n[INFO] Using {len(trends_data)} cached datasets. "
              "Delete scripts/output/gt_raw_*.csv to force re-fetch.")
