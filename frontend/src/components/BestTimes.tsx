      ...
      {bestTimes.map((slot, i) => {
        ...
        return (
          <motion.div
            ...
            className="flex items-center gap-2 sm:gap-3 rounded-lg bg-white/5 px-3 sm:px-4 py-2.5 sm:py-3"
            initial={{ opacity: 0, x: -16 }}
            animate={{ opacity: 1, x: 0 }}
            transition={{ delay: i * 0.08 }}
            whileHover={{ backgroundColor: "rgba(255,255,255,0.06)" }}
          >
            <span className="w-6 sm:w-8 text-xs sm:text-sm font-bold text-gray-400">
              #{i + 1}
            </span>
            <div className="w-20 sm:w-28">
              <span className="text-xs sm:text-sm font-medium capitalize text-gray-200">
                {slot.dayNameFull}
              </span>
              <span className="ml-1 text-xs sm:text-sm text-gray-400">{slot.hour}h</span>
            </div>
